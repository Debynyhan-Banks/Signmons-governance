...
Read-only permission inventory: no project bindings returned for this member; source bucket has only legacy project owner/editor/viewer bindings, no direct build-account grant; registry signmons has no explicit policy bindings. Inherited/deny policy evaluation was not exhaustively proven, but source-read failure is explicit. Therefore do not ask for source permission alone and blindly retry: the build also needs image push and log writing. Proposed next owner decision, **NOT applied**: grant existing account source-object read scoped to signmons_cloudbuild (roles/storage.objectViewer), image write scoped to existing us-east5/signmons registry (roles/artifactregistry.writer), and build log write in signmons (roles/logging.logWriter), preferably time-bound to the single build; no Owner/Editor, secrets, runtime identity, deployment or other roles. Exact conditions/removal must be reviewed before changes. Then separately authorize one fresh build under the same timeout/allowance. This is the existing item3 external permission gate, not new MVP scope. No scope deviation in executed actions.
