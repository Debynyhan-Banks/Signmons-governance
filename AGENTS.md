...
## Required checks
Governance: `node scripts/frozen-baseline-check.mjs`; `SIGNMONS_BACKEND_REPO=<resolved backend> node scripts/docs-consistency-check.mjs`; `node --test scripts/frozen-baseline-check.test.mjs scripts/execution-placement.test.mjs scripts/intelligence-alignment-check.test.mjs`.
Backend: relevant full/focused tests, lint, build, architecture, database and browser gates according to the card; `git diff --check` in both repositories. For documentation-only edits, run documentation/architecture/whitespace checks; do not present old runtime tests as new.
