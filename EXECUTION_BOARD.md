...
- [ ] APP-013 Twilio-backed notification center and transactional customer messaging (`SCR-APP-021`, `SCR-APP-022`, `SCR-TECH-005`)
  - Fixed/versioned SMS lifecycle/state checks, read-only history UI and durable technician enqueue recovery are review-ready on backend PR #21. Appointment durable intents, operator recovery policy/UI, remaining events/template controls, technician notifications, email and live acceptance remain open.
