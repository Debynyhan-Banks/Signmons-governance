...
## Current progress checkpoint (2026-09-09, consent/expiry proposal for review)
...
Latest section is documentation only: [consent/expiry proposal v1](APP013_EMAIL_CONSENT_EXPIRY_PROPOSAL.md). D1-D4 propose explicit per-job consent, separate mailbox verification, 24-hour event expiry capped at active window start, and grant closure/90-day evidence retention. These choices require owner approval; no schema, capture, verification or positive admission is implemented. New verification work and retention compatibility may increase the existing allowance; size them before promising an ETA. Current validation is docs/placement/link/architecture/diff only. The following implementation evidence is from aa4c547, not rerun for this proposal.

All three immutable appointment event kinds now feed an inactive read-only diagnostic binding current job/policy and the retained recipient in one consistent database snapshot. Matching CREATE read-back is covered; cancellation retains its prior immutable window. Every result remains blocked because approved consent evidence and expiry policy are absent; no positive admission, credentials, queue or transport. Evidence: backend email-eligibility/ and readiness-report.md; 42 new tests, 1232 backend/170 UI tests, 23 invariant-checked reads and a real revocation race/write refusal. Existing 18 local migrations/19 crash cases, browser regressions and four clean audits pass. Prior migration-before-code and old/in-flight cancellation compatibility still require separate release review.
