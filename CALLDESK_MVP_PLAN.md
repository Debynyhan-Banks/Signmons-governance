...
## Current progress checkpoint (2026-09-09, consent/expiry rules approved; not implemented)
...
Latest section is documentation only: [approved consent/expiry contract v1](APP013_EMAIL_CONSENT_EXPIRY_PROPOSAL.md). Owner approved D1-D4 without amendment: explicit per-job consent, separate mailbox verification, 24-hour event expiry capped at active window start, and grant closure/90-day evidence retention subject to retention/hold compatibility. No schema, capture, verification or positive admission is implemented. New verification work and retention compatibility may increase the existing allowance; size them before promising an ETA. Current validation is docs/placement/link/architecture/diff only. The following implementation evidence is from aa4c547, not rerun for this approval record.
