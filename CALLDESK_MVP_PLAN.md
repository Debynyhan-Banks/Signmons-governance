...
## Current progress checkpoint (2026-09-09, protected customer intake isolation)
...
Latest section isolates fresh protected customer sessions from legacy intake and adds inactive credential-bound encrypted capture. A server marker and shared locks prevent the two legacy triage paths and direct capture/question helpers from adopting protected sessions. Replay preserves one capture/audit; expiry after persistence rolls back; replaced address refuses. Lost/expired sessions clear private form state and restart only as a fresh scope in a fictional browser fixture. Full protected AI intake and production transport remain unfinished. See [APP013_CUSTOMER_SESSION_SECURITY_PLAN.md](APP013_CUSTOMER_SESSION_SECURITY_PLAN.md). Validation: 1331 backend/170 UI tests, ten new local database checks, desktop/mobile expiry/loss proof, existing 19-migration/crash/consent/eligibility/browser regressions and four clean audits. No new schema, production route/key/configuration, collection, verification, event admission, queue or sending.
