...
## Current progress checkpoint (2026-09-09, local customer-session and consent model)
...
Latest section adds a local-only customer-session credential and exact mailbox/prompt-bound response model on the inactive consent evidence foundation. The owner approved implementation/testing, not activation. It creates fresh server-owned sessions, rejects integration keys/caller session IDs as customer authority, requires separate mailbox confirmation for grant, and persists the response atomically. Security/default-lifetime/key/transport limits: [APP013_CUSTOMER_SESSION_SECURITY_PLAN.md](APP013_CUSTOMER_SESSION_SECURITY_PLAN.md). Validation: 1305 backend/170 UI tests, lint/build/architecture/Prisma, 18 new local session checks and fictional desktop/mobile browser proof; existing 19-migration/19-crash/24-consent/23-eligibility regressions and four clean audits. No production key loader/fingerprint adapter, active route, collection, mailbox verification, event-time grant binding, expiry admission or sending.
