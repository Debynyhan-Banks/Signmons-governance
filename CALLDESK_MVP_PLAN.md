...
## Current progress checkpoint (2026-09-09, inactive consent evidence and job binding)
...
Latest section implements the inactive evidence foundation under the [approved consent/expiry contract v1](APP013_EMAIL_CONSENT_EXPIRY_PROPOSAL.md): immutable scope and GRANTED/DECLINED/REVOKED revisions, encrypted mailbox/fingerprint fields, transaction-local receipt validation and one-time job binding. No production customer authentication boundary or fingerprint adapter/key is provided. Collection, verification, event-time grant binding, expiry enforcement, positive admission and sending remain absent. Validation: 1261 backend/170 UI tests, lint/build/architecture/Prisma, 19 local migrations, 24 new consent checks, existing 19 crash cases and 23 eligibility reads, local Settings/Inbox/composition browser proofs and four clean audits. Independent evidence-parent retention is RESTRICTed; release compatibility remains unresolved.
