...
Signmons Intelligence is the governed reasoning, context, knowledge, advisory, tool-orchestration, evaluation and improvement layer shared across Signmons. Reporting is one output. Logical capabilities include CSR/intake, HVAC context, safety, Comfort Advisor, ethical sales assistance, customer memory, approved knowledge retrieval, scheduling, payment, dispatch/escalation and evaluation. These are not separate products or a requirement for separate model calls.

The orchestrator combines authorized context with policy and approved knowledge, proposes typed tool actions and records actual outcomes for technician briefing and independent review. Reviewed outcomes can inform improvement only under rights, retention, evaluation and explicit training/release approval. HVAC context is not autonomous technical diagnosis. Full APP-021/024 pricebook/diagnostic products do not block basic advisory discovery; their own acceptance remains mandatory for those capabilities.
