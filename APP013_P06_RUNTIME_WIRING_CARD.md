...
Item 2 implementation detail, owner continuation "i reviewed proceed", source backend c4751d4750b37d834691838c72bf235ae8a62809 / governance e30af2a: reuse the loaded runtime, existing Nest Prisma/cipher, narrow HTTP/page factories and Docker-packaged assets. Add controlled-intake-startup.ts/spec and connect main before parsers/listen. Server-only CONTROLLED_INTAKE_RUNTIME_JSON defaults absent/disabled; bind facts from NODE_ENV, GOOGLE_CLOUD_PROJECT, K_SERVICE/K_CONFIGURATION/K_REVISION, PORT and the six explicit false flags. Separately injected CONTROLLED_INTAKE_SECRETS_JSON maps exact numeric references to hex strings (32-byte purpose keys; existing 32-hex Twilio token); validate exact keys/encoding, copy and zeroize temporary buffers, never log contents or fetch secrets. Release review must attest actual secret-version-to-material bindings; labels alone are not proof. Read only the two fixed Docker-packaged asset files once after enabled validation, reject missing/empty assets, construct once and mount only after successful startup. Disabled mode accesses no intake resources/secrets/assets; invalid enabled mode refuses before listen. Test absent/disabled/malformed configuration, resource/asset failures and sanitized errors, main mount ordering, plus the existing eight real HTTPS/disposable-DB browser cases through this same startup helper with synthetic injected resources. Full existing gates. No schema/resource/provider configuration or secret access is authorized. Observable finish: item 2 locally complete, leaving only fixed items 3 and 4; no deployment or acceptance claim.
...
Fixed remaining owner-facing checklist: (1) browser phone-code -> address -> job, (2) default-disabled startup wiring, (3) release packet/review, (4) approved capped staging test/acceptance. These are concrete remaining tasks inside P06, not new acceptance packages. Current turn targets only (1) from 94e81be/a43865e: reuse existing page controls and loader, bootstrap safe notice version/text; controlled START/CHECK only, locked phone after START and explicit end to change it, no replacement attempt/resend. Guarded local HTTPS browser verifier must use actual loader/services/DB/HTTP/page with synthetic external ports, never pre-provision phone proof. Test 390/1440 wrong/correct code, accepted lost-ack exact retry, outside/unknown/correction, close/reload/privacy/job counts. Full card gates; no live action. Startup registration remains (2).

Session-close checkpoint from dd10b14/703bf76: reuse VerificationCleanupService explicit close/purge with a new server-bound CONTROLLED_SESSION_V1 tenant mode (sweeps/reference purges remain fixture-only). Add separate controlledLifecycle transport port and bootstrap sessionCloseAvailable capability; page enables existing forget/end only when this server capability is present. Closure commits before purge, failure returns cleanupPending, exact repeated end remains safe; retain submitted business records, immutable consent, monetary holds, request counters and audits. No scheduler, schema, new page or new route. Tests actual loaded runtime close/failed purge/retry, foreign tenant refusal, no further verification; existing connected browser close at 390/1440 preserves admitted jobs and clears abandoned verification payloads. Full existing gates. This is a bounded part of item 2; actual browser phone-code entry and default-disabled startup registration remain unfinished, not new sections. No production action.

Loader checkpoint from 67923b3/7974319: implement controlled-intake-runtime.ts/spec with raw envelope validation before injected secret material use; no secret fetching/environment/main registration. Require current ACTIVE tenant settings.controlledRuntimeApproval exactly enabled/digest matching envelope, database clock and current organization/payment/category authority; no approval writer. Construct existing actual services once per returned binding, inject dedicated fingerprint, same shared budget and phone liability admission, actual adapters (synthetic external ports in tests). Current runtime approval checked before budget/body, phone policy and final admission; exact committed receipt replay remains in existing composition. Validate fixed numeric references, 32-byte distinct injected purpose keys and Twilio token, refuse projects other than signmons because existing Google transport fixes that quota project. Return frozen binding plus irreversible local retire; no fixture ports. Controlled end remains closed until next connected lifecycle/browser work. Extend existing guarded disposable DB verifier with loader startup, current approval revocation, transport START and synthetic START/CHECK; no positive approval mocks or live calls. Full existing gates required. Three P06 items unchanged; no new permission requested or release authority implied.

Owner decision: "yes, i approve" approves the bounded fingerprint design below for disabled implementation. Source backend 06558e2 / governance f7e0c44. This supersedes NOT APPROVED and the design-review pause below, not the separate secret provisioning/release gates. Implement dedicated adapter/spec with purpose+key-version+tenant+normalized-mailbox JSON serialization, copied private key bytes and explicit retirement (zeroize/refuse); add fingerprintKey/fingerprintKeyVersion envelope fields and reject aliasing another secret resource even at another version. Replace only the existing connected local harness's test fingerprint function with the actual adapter using synthetic bytes; preserve all immutable evidence. Run full card checks including connected database/browser regressions. No scope deviation; loader and browser phone flow remain within existing wiring item.

2026-09-14 loader entry inspection at backend f787d4f / governance db417aa found an unresolved security-plan approval boundary before implementation. AppointmentEmailConsentEvidenceStore requires EmailConsentFingerprint; only synthetic scripts implement it with fixture-only keys. APP013_CUSTOMER_SESSION_SECURITY_PLAN.md item 3 and DATA_CONTRACTS.md explicitly require separately reviewed dedicated key lifecycle/domain separation before adapter implementation. This card's secret envelope currently names session keys, verification digest and Twilio token only. No production fingerprint adapter/key reference exists. Therefore loader implementation has not started; do not supply the fixture or reuse another key to bypass this gate.

Proposed bounded repair inside existing P06 runtime wiring (NOT APPROVED): add a dedicated 32-byte HMAC-SHA-256 mailbox fingerprint adapter, serialize purpose `signmons.email-consent.fingerprint.v1`, tenant ID and the existing normalized mailbox unambiguously, and record an explicit key version. Add a distinct numeric secret reference/key-version field to the runtime envelope; never reuse signing, verification, encryption or provider keys. Freeze/copy injected bytes, refuse missing/invalid authority, and return no fingerprint in customer receipts. Rotation affects new evidence only; preserve historical immutable evidence and its key version, never compare different versions as equal or automatically rewrite old records. Compromised-key use must fail closed; no automatic fallback. Exact mailbox normalization remains unchanged. Tests: deterministic same tenant/version, cross-tenant/purpose/version separation, malformed key/reference refusal, no key aliasing, preserved immutable records and existing consent/database/browser regressions. Expected files: dedicated adapter/spec, runtime envelope/spec, then already-planned loader/spec; no schema/new route/package. No actual key creation, secret access/configuration, deployment, sending or paid test is authorized. Owner decision required: approve this security design for disabled implementation; actual secret provisioning remains a later release approval. Alternative of omitting consent or reusing fixture keys would weaken the existing journey and is not recommended. Three remaining P06 items and accepted counts stay unchanged; this is a discovered prerequisite within wiring, not an added acceptance section.

Runtime configuration/ingress checkpoint continues approved items 1 and 3 from backend e2fa320 and governance 01fd777. Reuse strict activation/phone policy parsers and the pre-parser mount. Add a pure exact-envelope validator and process-local opaque managed-HTTPS binding, requiring exact runtime facts, six disabled safety flags, matching packet/tenant/window, finite budgets and numeric same-project secret references. Invalid enabled input throws sanitized startup refusal; absent/false remains unbound. Tests cover configuration mutation/mismatch/expiry and actual HTTP host/opaque-binding refusal before transport. No secrets are fetched, clients constructed, loader registered or release authorized. Existing full database/browser gates remain required; full same-page phone journey and loader remain unfinished under this card.

Phone-spend admission detail: new ControlledCustomerAdmission implements existing VerificationAdmission, no new route/schema. Strict server-only packet/tenant/account/service/participant-HMAC/notice/rate/window/flow/account-ceiling policy; immutable digest must match enabled controlledPhoneApproval in current ACTIVE tenant settings. Same staging-phone account advisory lock includes old staging and new controlled held liabilities, never resets/refunds. First valid START atomically binds packet to authenticated session/conversation and exact operation/participant; CHECK/replay require same hold/current approval. Existing durable service supplies one START/five CHECKs and same-transaction reservation before provider call. Tests strict policy/default refusal, actual disposable DB concurrency/rollback/old holds/expiry/revocation/cross-session and unchanged existing suite. No setting writer/real approval/rate/cap invented; current code remains unregistered. Runtime composition and full browser code entry remain pending.
