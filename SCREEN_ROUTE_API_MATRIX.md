...
| `SCR-APP-021` | `/app/notifications` | backend app surface | `GET /communications/sms/history`; `GET /communications/sms/enqueue-intents`; API-only owner/admin `POST /communications/sms/enqueue-intents/:intentId/retry` | APP-013 Transactional SMS Lifecycle Contract | APP-013 | read-only UI and exhausted-intent retry API in review; recovery UI/live acceptance pending |
