...
| `SCR-APP-006C` | `/app/dispatch` (payment-gate slice) | backend app surface | `GET /jobs/dispatch-board`, `GET /jobs/dispatch-board/:jobId`, `POST /jobs/:jobId/assignments` | APP-012 payment-before-dispatch gate contract | APP-012 | bounded implementation review-ready; ticket remains active |
| `SCR-TECH-001`, `SCR-TECH-002`, `SCR-TECH-003` | `/app/technician` | backend app surface | `POST /jobs/technician-links/:technicianId`, `GET /technician/jobs`, `GET /technician/jobs/:jobId`, `POST /technician/jobs/:jobId/status` | technician mobile job workflow contract | APP-009 | implementation review-ready; release pending |
