...
| `SCR-APP-021` | `/app/notifications` | backend app surface | `GET /communications/sms/history`; `GET /communications/sms/enqueue-intents` API-only pending UI integration | APP-013 Transactional SMS Lifecycle Contract | APP-013 | read-only SMS history and technician intent API in review; recovery UI/live acceptance pending |
