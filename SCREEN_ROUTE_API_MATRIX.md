...
| `SCR-APP-021` | `/app/notifications` | backend app surface | `GET /communications/sms/history`; `GET /communications/sms/enqueue-intents`; `GET /communications/sms/capabilities`; owner/admin `POST /communications/sms/enqueue-intents/:intentId/retry` | APP-013 Transactional SMS Lifecycle Contract and Unfinished Calendar Consumer Guards | APP-013 | guarded retry and Calendar hold labels in review; owner/live acceptance pending |
| `SCR-APP-022` | `/app/messaging-settings` | backend app surface | owner/admin `GET /communications/customer-messaging-settings`, `PUT /communications/customer-messaging-settings`; existing operator `POST /communications/sms/transactional` separate | APP-013 Customer Messaging Settings Contract | APP-013 | implemented in source, review-ready; four fixed customer SMS previews and event preferences, no delivery activation/deployment |
| `SCR-TECH-005` | `/app/technician` Notifications panel | backend app surface | `GET /technician/notifications` with existing signed technician link; no inbox mutations | APP-013 Technician Notification Inbox Contract | APP-013 | implemented in source, review-ready; current-assignment recorded activity only, not delivery/read history; no deployment |
