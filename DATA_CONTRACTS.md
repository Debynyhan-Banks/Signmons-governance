...
- A same-transaction audit establishes persistence linkage, NOT external customer authentication. A reviewed future customer boundary must authenticate session authority and bind the actual displayed mailbox/prompt/response before producing the receipt. Existing address-only capture, SMS or owner approval cannot supply it. There is no route, DI registration, caller, collection UI or operator-grant path.
