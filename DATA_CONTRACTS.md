...
Approved design, not implemented: [APP013_EMAIL_CONSENT_EXPIRY_PROPOSAL.md](APP013_EMAIL_CONSENT_EXPIRY_PROPOSAL.md). Owner approved D1-D4 without amendment on 2026-09-09: per-job explicit permission, separate mailbox verification, 24-hour event deadlines (active emails capped at window start), grant termination and 90-day post-close evidence retention. Existing diagnostic behavior below is unchanged. Design approval supplies no customer authority; historical events remain blocked, without backfill. A separately bounded implementation is still required before these rules can be enforced. Verification challenge/transport and retention/hold compatibility remain gated; no sending or purge is authorized.
...
- No implemented trusted email-consent authority or configured event-expiry policy exists. Approved D1-D4 define the design, not actual customer evidence or runtime authority. An address, owner review, tenant permission, SMS consent or hypothetical CustomerProfile.consentFlags field is not substituted. No positive path, configured TTL, consent collection or storage model is introduced in this approval checkpoint.
