...
## Current: P05 active — disabled HTTP mount validated; UI proof remains

P05 active: customer-session HTTP middleware is mounted in main.ts before CORS/default parsing, disabled without server resources. Streaming transport validates ingress/context before body consumption and retains the 16 KiB bound and existing budget. Only the customer namespace is intercepted; unrelated webhook bytes and rawBody:true preserved. Direct socket TLS facts only; forged forwarded headers refuse. Seven new tests including actual local HTTP; 2,294 passed (3 skipped), build/lint/architecture, zero-vulnerability audits and existing disposable database/browser regression passed. evidence/APP-013/p05-http-mount.md. Local adapter/composition/closed mount implemented; customer UI/item 3 and connected desktop/mobile proof/item 4 remain before P05 review readiness. No live configuration or managed-ingress qualification claimed; P06 retains reviewed ingress/resources/release gates. Accepted packages 3/60 (5%), walkthrough 3/8 (37.5%) and low-confidence 60–84 productive-day baseline unchanged. No admin implementation or new package. No scope deviation.
