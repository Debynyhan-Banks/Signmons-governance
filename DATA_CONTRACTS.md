...
- A changed technician `on_my_way` action queues the fixed customer template after the job/audit transaction commits. No-op retries, rejected writes and failed commits do not queue. Queue/logging failure cannot reverse committed technician status; failure logging excludes raw errors and message/customer credentials.
- On-the-way state identity includes `technicianStatusUpdatedAt` to distinguish departure episodes for the same assignee. Earlier on-the-way hashes without this field fail closed; appointment hashes are unchanged. Post-commit enqueue is not a durable outbox: crashes/queue failures can leave status without a notification and require operator-reviewed recovery, not automatic status replay.
