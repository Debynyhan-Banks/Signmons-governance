...
- Fresh UNCERTAIN is the bounded active-attempt marker. Reconciliation returns pending before job lookup, Calendar access or persistence until ten seconds after the journal attempt timestamp. The reviewed Google creator starts its eight-second abort deadline before credential acquisition and checks it before fetch; after that deadline it cannot initiate or continue a client write. Provider-side processing after abort remains outside the database boundary.
