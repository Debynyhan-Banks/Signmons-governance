...
- After Calendar acknowledges initial insertion, database finalization conditionally matches the tenant/job ACCEPTED reservation and atomically persists the calendar reference, confirmation SMS intent and `appointment.initial_confirmed` customer audit containing intent linkage only. Queue/operations-notification/logging failure after that commit cannot undo booking. Successful completed reschedule still queues directly after the canonical operation and retains its post-commit gap. Cancellation finalization is defined below.
...
- Durable capture now covers initial appointment finalization, post-Calendar cancellation finalization and technician departures. Reschedule still has a post-commit enqueue gap; no historical backfill is included. Owner/admin retry of exhausted intents is defined below; this does not override suppression or replay a queued/dead-letter event. Intent recovery permits these three templates only, validating the recorded state hash before queue insertion.
