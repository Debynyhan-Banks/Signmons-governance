...
R08 disposable control-plane reset qualification completed2026-09-16 08:42–08:50EDT. Owner performed one canary reset; metadata/statistics show password ALTER1row/1call with SCRAM password field, no explicit LOGIN token; NOLOGIN/NOINHERIT/no elevation/memberships0/limit0/past expiry preserved. Positive CREATE control1row. Canary172125 and temporary pg_stat_statements1.12/OID172081 removed with guarded cleanup; independent role/extension/view absence verified. Real runtime163840 unchanged/disabled/limit10; trackingtop/utilityon unchanged. This supports the bootstrap-then-control-plane-reset candidate, not provider-wide secret-free logging or authentication proof: sensitive verifier retention was observed, never exported; final plaintext was not inspected. Prior rawSQL literal FAIL remains valid. No real runtime assignment/activation authorized. Next is review of this measured method and explicit residual-risk/credential-custody acceptance before any real-role execution; no repeat diagnostic, new helper, vendor-contact gate or section. R08-R12five; packages5/60,walkthrough3/8 unchanged; ETAunvalidated. No scope deviation beyond expressly approved disposable qualification.
...
R06 database-liability check now complete for the isolated child:0address operations,0held address micros,0request claims. Prior phoneUSD0.50/local GoogleUSD0.20 holds remain separate and retained. Current Cloud Run metadata preserves traffic/disabled flags;13existing secret resource names, no proposed P06 purpose resources yet. Infrastructure worksheet now includes regional compute, logs, storage and secret costs with assumptions; final exact bindings/authorization and release allowance remain to assemble before R07. See latest evidence section; no new task, provisioning or billing action.

Latest R06 readbacks: Google Address Validation enabled; effective validate_address_requests quotas5/minute and10/day. Old staging operator disabled with unchanged validSince; runtime service-account resource policy bindings empty. Project ancestor list contains only signmons; direct runtime project role is Firebase Auth Viewer. Usable runtime serviceusage.services.use authority is not established; Policy Troubleshooter API is disabled and was not enabled. Include exact least-privilege authorization in R07 review, never infer a grant from API enablement. See backend evidence/APP-013/p06-r06-qualification.md. No scope/count change or paid request.

Owner supplied Eternity rules: USD99 regular initial diagnosis deposit; USD150 replaces it for all after-hours emergencies including Sunday. Hours America/New_York: Mon-Fri07:00-19:00, Sat09:00-17:00, Sunday emergency-only. No guaranteed emergency dispatch; otherwise collect for business-hours follow-up. See governance ETERNITY_PILOT_OPERATING_RULES.md. Business-input questions are resolved; runtime policy approval/provisioning is not. Existing fixed policy cannot select after-hours replacement and enabling both amounts would sum them. Owner explicitly approved the regular-visit-only first controlled test using one USD99 deposit. This resolves the test-scope decision, not R07/R08 setup or R10 run authority. Read-only refresh confirmed both prior Google holds totalUSD0.20 and old stagingPhoneTokenSigner remainsDISABLED. No emergency-pricing implementation or added task approved.
...
R06 remains open. Existing Safari account/service readback now confirms Signmons SMS enabled, Fraud Guard on, United States SMS monitored and other listed country/voice traffic disabled; sign-in blocker cleared. Exact business fee/deposit and operating-hours/after-hours values are now supplied as recorded above; no fictional defaults or runtime activation inferred. Backend evidence/APP-013/p06-r06-qualification.md contains the proposed policy field map and participant notice. Remaining implementer work: Google/exception/liability refresh and final infrastructure-cost/binding qualification. Do not activate, reset holds, send, provision or deploy. Missing setup belongs to existing R07/R08 approval boundaries, not another coding task.
...
P06 fixed item 3 release packet is drafted in APP013_2B_ACTIVATION_PACKET.md, not complete or executable. Source backend 53037fbd118cc4547061dfaf373c45b20a05962b; items 1 and 2 locally complete. Fresh read-only staging metadata confirms old candidate 00065-guw/image 25e194ac, normal traffic 100% app013bounds, all six safety flags false and no new controlled startup variables; customer-purpose secret metadata filter returns no entries. Draft separates disabled candidate (no new intake material needed) from enabled run and records unresolved image/provenance, numeric secret/schema qualification, protected material injection and itemized cost gates. No build, deployment, secret access/configuration, IAM, real tenant query or provider call. Two fixed items still remain: 3 exact packet/owner review, 4 approved capped run/acceptance; no added package. Documentation-only checks; earlier 2,359 runtime tests are not new results. Evidence: backend evidence/APP-013/p06-release-packet-review.md. Accepted 5/60 (8.3%), walkthrough 3/8 (37.5%), low-confidence 4–8 weeks plus external waits unchanged. No scope deviation.

## Historical: P06 checklist item 2 locally complete — default-disabled startup
...
## Historical: P06 checklist item 1 locally complete — loaded browser journey

Fixed owner-facing checklist item 1 is locally complete: actual browser code request/wrong-correct CHECK, draft/address and job through the loaded runtime, real HTTPS mount/services/disposable DB, synthetic external ports only; no pre-provisioned phone proof. Eight cases at 390/1440 cover accepted lost-ack exact retry, outside, unknown, correction and close/reload; 24 synthetic phone and 10 synthetic address invocations across those cases. Full 2,342 tests (3 skipped), build/lint/architecture/schema, both zero-finding audits and full DB/browser gates passed; evidence/APP-013/p06-loaded-browser-journey.md. Remaining concrete checklist: 2 default-disabled startup wiring; 3 release packet/review; 4 approved capped staging test/acceptance. This reports three concrete tasks, not three broad sections or new packages. Main still unbound; no real provider calls/deployment. Accepted 5/60 (8.3%), walkthrough 3/8 (37.5%), provisional low-confidence 4–8 weeks at 25–30 collaborative hours/week plus external waits unchanged. No scope deviation.

## Historical: P06 controlled session close — partial runtime checkpoint

P06 controlled session close locally connected to loader and existing page. Explicit tenant-bound cleanup reuses close-before-purge; controlled close does not authorize sweeps/reference purge. Server bootstrap advertises close availability; page uses existing end/forget flow only with that capability. Actual DB tests prove foreign refusal, closure survives failed purge, retry purges verification payload, closed session refuses verification, holds retained. Six connected browser close cases at 390/1440 preserve jobs and clear private fields; uncertain pending submission retains existing local-clear behavior. Full 2,341 tests (3 skipped), build/lint/architecture/schema, both zero-finding audits and DB/browser gates passed; evidence/APP-013/p06-session-close.md. No live calls/secret actions/deployment. Remaining wiring: actual browser phone-code-to-job flow using loaded binding and default-disabled startup/asset registration. Same three P06 items; accepted 5/60 (8.3%), walkthrough 3/8 (37.5%), low-confidence 4–8 weeks at 25–30 collaborative hours/week plus external waits unchanged. No scope deviation.

## Historical: P06 runtime loader — partial runtime checkpoint

P06 explicit runtime loader now locally implemented: strict envelope/injected keys, current ACTIVE tenant controlledRuntimeApproval digest and DB expiry, actual current authority readers, consent/fingerprint/durable phone/address/shared-budget service composition, frozen binding and local retirement. No main/environment/secret-store registration. Guarded DB test exercises loader, actual protected START and synthetic durable START/CHECK, approval revocation and retirement; zero live calls. This caught/fixed controlled CHECK incorrectly forwarding fresh opt-in to the durable core; START-only opt-in preserved. Full 2,338 tests (3 skipped), build/lint/architecture/schema, both zero-finding audits and complete local DB/browser gates passed; evidence/APP-013/p06-runtime-loader.md. Remaining wiring: controlled session close, actual same-page phone-code-to-job journey through loaded binding, and default-disabled startup/asset registration before release review. Same three P06 items, accepted 5/60 (8.3%), walkthrough 3/8 (37.5%), low-confidence 4–8 weeks at 25–30 collaborative hours/week plus external waits unchanged. No scope deviation.

## Historical: P06 approved email fingerprint — partial runtime checkpoint
...
## Historical: P06 controlled phone boundary — partial runtime checkpoint
...
## Parallel planning checkpoint: P06 runtime-wiring card source-complete — implementation decision required

P06 item 2 pre-code inspection is complete in `APP013_P06_RUNTIME_WIRING_CARD.md` from backend `d07ff354` / governance `3c37077`. It maps exact loader, managed-HTTPS, controlled phone, shared admission/budget, same-origin page, lifecycle, retention and test boundaries. Source inspection found the P05 page is test-only and absent from the runtime image, no same-origin BFF is recorded, the browser budget is process-local, and existing phone admissions are fixture-only or tied to the closed fixed-session staging runner. No fixture seam may be promoted. Smallest owner decision: approve the recommended backend-served same-origin assets plus PostgreSQL-backed shared admission/budget and controlled customer admission adapter, or name an existing external BFF/load-balancer path. After that approval, implement and prove the same-page local phone-to-job flow; P06 items 3 release and 4 paid run remain separately gated. Documentation only; no code/runtime/schema/cloud/provider/customer change. Accepted packages 5/60 (8.3%), walkthrough 3/8 (37.5%); provisional 4–8 weeks at 25–30 collaborative hours/week plus external waits unchanged, low confidence. No scope deviation implemented.
...
## Historical: nullable location implemented and locally validated
...
## Historical: automatic admission acceptance adopted; implementation pending
...
## Historical: Google-only data adaptation; narrow admission lifecycle remains
...
Each saveDraft creates a replacement immutable version and immediately sets DRAFT, so prior eligibility is unavailable until new review; no silent fallback to an older policy. Suspension remains possible after the allowlist is withdrawn. Database triggers reject version edits/deletes; future retention/account deletion needs explicit qualification, not ad hoc deletion. All migrations in this section ran only in a disposable local database. Final evidence: 1,941 tests, six registry groups, fourteen existing durable/browser groups; prior runner exit 139 passed on rerun and remains documented.

Owner approved P1 with “i agree proceed”. Add inactive TenantSmsPolicyVersion (immutable tenant-bound public content, canonical digest and creation time) and TenantSmsPolicyHead (one current version per tenant, integer revision, lifecycle state and last audit reference). Composite foreign keys enforce tenant identity. Replacing a draft creates a new version and increments the head; reviewed content is never overwritten. State changes and privacy-safe audit commit together under a tenant policy lock with expectedRevision checks.

Application methods, not public routes: saveDraft accepts exactly expectedRevision and content; transition accepts exactly expectedRevision, target and evidenceRef. Content binds legalSender, purpose=APPOINTMENT_UPDATES_V1, supportEmail, disclosure/disclosureVersion, privacyUrl/privacyVersion, termsUrl/termsVersion, effectiveAt and expiresAt. Dates are canonical UTC strings, expiry follows effectiveness. A server-owned tenant URL allowlist governs exact HTTPS URLs; no network fetch or caller-supplied allowlist. Owner/admin context is mandatory; impersonation refuses.

DRAFT→REVIEWED→PUBLISHED_VERIFIED→CAPTURE_ELIGIBLE, or SUSPENDED from an existing state. Publication transition requires a trusted injected, exact tenant/version/digest/evidence/deadline-bound attestation, explicitly fixture-only in P1; real publication is refused. Reader rechecks current active tenant, state, dates, allowlist, content digest and publication attestation while holding the same head lock. It returns only public content plus opaque version/revision binding, fixtureOnly=true, liveCaptureEnabled=false and deliveryAuthorized=false. Expected binding revalidation rejects replacement/suspension. No route, DI registration, live consent write or delivery connection is added. P2 owns real-consent integration; fixtures are not promoted.
...
Durable phone ledger entries optionally contain freshnessPolicy and proof in the existing encrypted verificationOperations envelope. Legacy entries remain readable but no replay mints proof for an old approval. New configured START/CHECK policy must match; observation rechecks policy under the session lock, stores database-epoch timestamps only on approved success, and cannot overwrite prior null revocation. Browser verify body keys are unchanged; STATUS and REVOKE require empty operationId/code/startOperationId/noticeVersion, requested=false and current sessionToken/phone. They invoke no provider. Response is fixtureOnly=true, state=FRESHNESS, proof={state:CURRENT|NOT_CURRENT,checkedAt:number|null,expiresAt:number|null}; all existing phone/booking/delivery authority flags stay false. Successful CHECK/replay also projects current freshness when configured. Revocation is sticky, audited and serialized with observation; audit failure rolls back. No budget hold is released.

Address correction candidate and confirmation receipts add original checkedAt/expiresAt and confirmation adds stable confirmedAt. Confirmation does not renew validation. Trusted correction binding uses active session plus current approved organization/policy instead of coarse conversation.updatedAt, so unrelated phone ledger writes cannot revoke the address; exact address intent/revision remains separately bound. Local policy reads organizationProfileV1.approved.approvedAt under the caller's session lock; no customer-supplied version grants authority. Correction remains addressVerified=false, county=UNKNOWN, admissionAuthorized=false, deliveryAuthorized=false.

No module/controller production registration, migration or live provider initialization. Old-reader rollout for new optional encrypted metadata, cleanup/restore/retention and real-proof atomic job consumption remain 1B/2A/2B gates. Ninety-day reference retention is not approved by 1A. Source/network contracts and accounting amounts are unchanged.
...
AddressVerificationOperation retains Signmons account/tenant/conversation/session/intent/operation UUIDs, revision, policy-only hash, createdAt, heldMicros and optional claim attempt UUID. AddressVerificationRequest maps bounded logical request aliases to one operation. State is RESERVED, DISPATCH_CLAIMED or CANCELLED; OBSERVED/UNCERTAIN are schema-reserved for later work, not exposed transitions. Database checks prevent zero-cost claimed rows, claimed state without attempt, and cancelled rows with retained cost. Audits contain Signmons operation and transition only. No Google payload/identifier, code, bearer, address/unit or phone field.

Atomically lock account then tenant then existing customer session, check integer cost/request caps at all three scopes, reserve/create/audit, and commit. One claim wins; replay/restart cannot reclaim, and every receipt including the winner has dispatchAuthorized/addressVerified/admissionAuthorized/deliveryAuthorized false, fixtureOnly true, county UNKNOWN. Only RESERVED can cancel/release cost; request allowances and unresolved holds do not reset across months. No settlement, replenishment, provider I/O, scheduler, production retention/cleanup or admission path. New schema migration is approved only for disposable local tests; no production application authority.
