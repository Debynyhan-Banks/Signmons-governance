...
Pending contract proposal (not approved or active): [APP013_EMAIL_CONSENT_EXPIRY_PROPOSAL.md](APP013_EMAIL_CONSENT_EXPIRY_PROPOSAL.md). D1-D4 propose per-job explicit permission, separate mailbox verification, 24-hour event deadlines (active emails capped at window start), grant termination and 90-day post-close evidence retention. Existing diagnostic behavior below is unchanged. Do not infer policy approval, add grant bindings to historical events, configure these values or activate sending until the owner reviews the proposal and a separately bounded implementation is approved. Verification transport/challenge design and retention/hold compatibility remain gated.
...
- No approved email-consent authority or event-expiry duration exists. An address, owner review, tenant permission, SMS consent or hypothetical CustomerProfile.consentFlags field is not substituted. No positive path, arbitrary TTL, new consent collection or storage model is introduced. Next owner decision: approve mailbox/tenant/purpose-bound evidence and revocation/recipient-change handling, plus event-age/expiry semantics, before positive admission work.
