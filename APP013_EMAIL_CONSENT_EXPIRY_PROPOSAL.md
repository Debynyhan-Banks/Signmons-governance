...
Integrated protected intake and local human-reviewed job admission are review-ready. Optional grant/decline/no-email history is atomically associated with the CREATED job when present, but never authorizes sending; no evidence remains allowed. The unregistered admission does not implement email eligibility, transport or production split ownership. Next proposed: a durable customer/operator review-request boundary without exposing the customer bearer credential. No Calendar/payment/provider activation; mailbox verification, expiry/delivery admission, retention and delivery remain unfinished.
