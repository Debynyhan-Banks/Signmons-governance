...
The inactive evidence/job-binding, session/prompt-response model and protected intake isolation/capture are review-ready. Production integration and full credential-bound AI intake remain absent. Next proposed bounded section: local bootstrap origin/CSRF/request-limit/private-response protections without live route/configuration or collection. Read APP013_CUSTOMER_SESSION_SECURITY_PLAN.md before any production key adapter, route or transport work. No verification, queue admission, appointment credentials or sending is authorized.
