...
test('added section fails',()=>{const f=files[2];assert.ok(compare({...baseline,[f]:baseline[f]+'\n## 3E — extra work'},baseline).length);});
test('acceptance dependency mutation fails',()=>{const f=files[2];assert.ok(compare({...baseline,[f]:baseline[f].replace('**Entry:** 2B accepted','**Entry:** 2B optional')},baseline).length);});
